...
# ==================== Eval Wrapper ====================
def run_eval(model_builders, tag, data, devices, dataset, config=None, batch_size=1, workers_per_device=1, loader_workers=0):
    workers_per_device = max(1, workers_per_device)
    loader_workers = max(0, loader_workers)
    if devices:
        logger.info(f"{dataset.upper()} | {tag:10}  Using GPUs (>=50% free): {devices} with {workers_per_device} worker(s)/device")
    else:
...
MODEL_FACTORIES = {
    ...
    # ),    "kimi-vl-a3b": make_model_factory(
    #     AutoModelForCausalLM,  # keep your factory shape; swap to AutoModelForVision2Seq if you wire images
    ...
    #         "use_safetensors": True,
    #     }
    ),    "qwen2.5-14b": make_model_factory(
        AutoModelForCausalLM,
        "unsloth/Qwen2.5-14B",  # alt: official instruct variant exists under Qwen/* if you need that
        tokenizer_kwargs={"use_fast": True},
        model_kwargs={
            "torch_dtype": torch.float16,
            "trust_remote_code": True,
...
def main():
    ...
    parser.add_argument("--workers_per_device", type=int, default=1, help="Number of parallel evaluation workers per GPU")
    parser.add_argument("--loader_workers", type=int, default=2, help="Number of DataLoader workers feeding each evaluation loop")
